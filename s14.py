...
def comparar_frase():
    ...
    if ast:
        nueva_arbol_semantico = construir_arbol_semantico(ast)
        evaluar_arbol_semantico(nueva_arbol_semantico)

        # Comparar con la base de conocimientos
        for proposicion in base_conocimientos:
            tokens_conocimiento = analisis_lexico(proposicion)
            ast_conocimiento = analizar_sintactico(tokens_conocimiento)
            arbol_semantico_conocimiento = construir_arbol_semantico(ast_conocimiento)
            evaluar_arbol_semantico(arbol_semantico_conocimiento)

            # Si alguna proposición en la base de conocimientos tiene un valor de verdad contrario a la nueva frase, hay una contradicción
            if arbol_semantico_conocimiento.truth_value is not None and nueva_arbol_semantico.truth_value is not None:
                if arbol_semantico_conocimiento.truth_value != nueva_arbol_semantico.truth_value:
...
def realizar_analisis():
    ...
    guardar_base_conocimientos(proposiciones)
