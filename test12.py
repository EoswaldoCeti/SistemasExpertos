...
def generar_arbol(ventana, a, b, c, resultado):
    ...
    # Función para determinar el color de cada nodo
    def determine_colors(a, b, c):
        colors = {}
        path = []  # Almacenar el camino recorrido

        if a:  # A es positivo (1)
            colors["A"] = "green"
            path.append("A")
            if b:  # B es positivo (1)
                colors["B1 = 1"] = "green"
                path.append("B1 = 1")
                if c:  # C es positivo (1)
                    colors["C3 = 1 (Salida=1)"] = "green"
                    path.append("C3 = 1 (Salida=1)")
                else:  # C es negativo (0)
                    colors["C2 = 0 (Salida=0)"] = "green"
                    path.append("C2 = 0 (Salida=0)")
            else:  # B es negativo (0)
                colors["B0 = 0"] = "green"
                path.append("B0 = 0")
                if c:  # C es positivo (1)
            ...
            if b:  # B es positivo (1)
                ...
                path.append("B0 = 0")
                if c:  # C es positivo (1)
                    colors["C2 = 1 (Salida=0)"] = "green"
                    path.append("C2 = 1 (Salida=0)")
                else:  # C es negativo (0)
                    colors["C2 = 0 (Salida=0)"] = "green"
                    path.append("C2 = 0 (Salida=0)")

        # Colorear nodos no visitados en negro
        for node_name in ["A", "A=0", "A=1", "B0 = 0", "B0 = 1", "B1 = 0", "B1 = 1", 
                          "C0 = 0 (Salida=0)", "C0 = 1 (Salida=0)", 
                          "C1 = 0 (Salida=0)", "C1 = 1 (Salida=0)", 
                          "C2 = 0 (Salida=0)", "C2 = 1 (Salida=0)", 
                          "C3 = 0 (Salida=0)", "C3 = 1 (Salida=1)"]:
