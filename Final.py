...
def generar_arbol(operacion, ventana, a, b, c, resultado):
    ...
    color_c0 = 'green' if not c and not a and not b else 'black'
    color_c1 = 'green' if c and not a and not b else 'black'

    color_c0_der = 'green' if a and b and not c else 'black'
    color_c1_der = 'green' if a and b and c else 'black'


    color_a0_or = 'green' if not a else 'black'
    color_a1_or = 'green' if a else 'black'
    
    color_b0_or = 'green' if not b else 'black'
    color_b1_or = 'green' if b else 'black'
    
    color_c0_or = 'green' if not c and not a and not b else 'black'
    color_c1_or = 'green' if c and not a and not b else 'black'

    color_c0_der_or = 'green' if a and b and not c else 'black'
    color_c1_der_or = 'green' if a and b and c else 'black'

    # Crear el árbol de estados básico con matplotlib
    if operacion == "AND":  # Conjunción (∧)
        ax.text(0.5, 0.9, "A", fontsize=14, ha='center', color='black')
        ax.text(0.3, 0.75, "A0", fontsize=12, ha='center', color=color_a0) 
        ax.text(0.3, 0.65, "B0", fontsize=12, ha='center', color=color_b0)
        ax.text(0.7, 0.75, "A1", fontsize=12, ha='center', color=color_a1)  
        ax.text(0.7, 0.65, "B1", fontsize=12, ha='center', color=color_b1)
        ax.text(0.2, 0.5, "C0", fontsize=12, ha='center', color=color_c0)
        ax.text(0.4, 0.5, "C1", fontsize=12, ha='center', color=color_c1)
        ax.text(0.6, 0.5, "C0", fontsize=12, ha='center', color=color_c0_der)
        ax.text(0.8, 0.5, "C1", fontsize=12, ha='center', color=color_c1_der)

        ax.plot([0.5, 0.3], [0.85, 0.75], 'k-')  
        ax.plot([0.5, 0.7], [0.85, 0.75], 'k-')  
        ax.plot([0.3, 0.2], [0.65, 0.5], 'k-')  
        ax.plot([0.3, 0.4], [0.65, 0.5], 'k-')  
        ax.plot([0.7, 0.6], [0.65, 0.5], 'k-')  
        ax.plot([0.7, 0.8], [0.65, 0.5], 'k-')  
        ...
        ax.text(0.5, 0.9, "A", fontsize=14, ha='center', color='black')
        ax.text(0.3, 0.75, "A0", fontsize=12, ha='center', color=color_a0_or) 
        ax.text(0.3, 0.65, "B0", fontsize=12, ha='center', color=color_b0_or)
        ax.text(0.7, 0.75, "A1", fontsize=12, ha='center', color=color_a1_or)  
        ax.text(0.7, 0.65, "B1", fontsize=12, ha='center', color=color_b1_or)
        ax.text(0.2, 0.5, "C0", fontsize=12, ha='center', color=color_c0_or)
        ax.text(0.4, 0.5, "C1", fontsize=12, ha='center', color=color_c1_or)
        ax.text(0.6, 0.5, "C0", fontsize=12, ha='center', color=color_c0_der_or)
        ax.text(0.8, 0.5, "C1", fontsize=12, ha='center', color=color_c1_der_or)
    ...
    # Agregar etiquetas de operación y resultado
    ax.text(0.5, 0.95, f"Operación: {operacion}", fontsize=14, ha='center', color='blue')
    ax.text(0.5, 0.45, f"Resultado: {resultado}", fontsize=12, ha='center', color='red')
